...
## 5. Index/Trigger/Procedure
    - How
    - What
    - benefit

## 6.   Privilege and Role

#### Privilege

Oracle 資料庫可以針對每一位使用者 (User/Schema) 給予特定的使用權限。例如 CREATE TABLE SELECT ANY TABLE 等操作，這些權限就是該使用者擁有的 Privilege 。如果GRANT 給 PUBLIC 等同於給所有人，屬於風險高的行為 。收回權限用 REVOKE


如果多個人都有相同的權限，對於 DBA 來說 一個一個去 GRANT 會花費大量的人力和時間。因此 ROLE 誕生了。

#### ROLE 

是一組權限的集合，一種權限的包裝和管理單位。 DBA 可以先將多個權限彙整成一個角色，再將角色授予特定使用者
