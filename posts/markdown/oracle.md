...
2. 邏輯結構 : 記錄資料庫邏輯物件的定義與關聯，是一種抽象結構。主要目的是為了讓資料庫的空間更有效率的使用 [[1]](#2)。例如: Tablespace, Segment, Block。
...
- SGA : 多種不同的共用記憶體元件組合的區塊，但下列三種是必要的:
    ...
    - Log Buffer : 用來記錄每一筆 SQL 的 Redo Entry，是為了當災難發生時可以進行 Instance Recovery[[1]](#1)。
...
- Background Processes : 負責資料庫的各種處理，下列五種是必要的:
    ...
    - DBWR (Database Writer): 將 Buffer Cache內的  Dirty Cache [[2]](#2) 寫回 DataFile
...
因為實體結構中最小單位為一個檔案，但對於資料庫來說太大，因此
